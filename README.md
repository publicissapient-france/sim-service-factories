...
Vertx est un framework polyglotte qui permet de faire communiquer facilement des services à travers [un bus d'évènements](http://vertx.io/core_manual_js.html#the-event-bus) (entre autres). Dans vertx, chaque service est appelé [Verticle](http://vertx.io/manual.html#verticle).
...
Pour ce faire, la première chose à faire dans votre Verticle est de [publier un évènement](http://vertx.io/core_manual_js.html#publishing-messages) **hello** dans le format suivant:
...
Maintenant, il vous faut indiquer toutes les 500 ms que votre factory est en vie. Heureusement, vertx propose un [ensemble d'outils](http://vertx.io/core_manual_js.html#periodic-timers) permettant de faire cela facilement.
...
Le fichier de configuration factory.json est utilisé par défaut avec votre factory. Si vous souhaitez variabiliser des élements de votre application, [vous pouvez l'utiliser facilement](http://vertx.io/core_manual_js.html#getting-configuration-in-a-verticle)
...
Pour recevoir ces messages, il vous faut [écouter](http://vertx.io/core_manual_js.html#registering-and-unregistering-handlers) sur l'adresse /city/factory/votre-id

Une farm peut vous faire une offre avec un stock inférieur à votre demande si elle ne peux pas y répondre intégralement.

Ce message est avec [timeout](http://vertx.io/core_manual_js.html#request-timeouts) vous devez donc [répondre](http://vertx.io/core_manual_js.html#replying-to-messages) assez rapidement que vous êtes intéressé:
...
Pour répondre aux demandes des stores, vous devez [envoyer](http://vertx.io/core_manual_js.html#sending-messages) une offre au store sur son adresse privée /city/store/id-store:
